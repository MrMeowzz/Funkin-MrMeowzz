...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
				  ...
				  case 'ugh' | 'guns' | 'stress' | 'no among us':
				  {
					  ...
					  var tank:FlxSprite = new FlxSprite(300, 300).loadGraphic(Paths.image('tankRolling'));
		...
		switch (SONG.player2)
		{
			case 'gf' | 'gf-christmas' | 'gf-car' | 'gf-pixel' | 'gf-tankmen' | 'speakers':
		...
		switch (curStage)
		{
			...
			case 'tank':
				...
				else if (SONG.player2 == "tankmannoamongus")
				{
					dad.y += 250;
				}
				else
		...
		ratingTxt = new FlxText(accuracyTxt.x + 85, accuracyTxt.y - 25, 0, "", 20);
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'tutorial' | 'bopeebo' | 'fresh' | 'dadbattle':
					if (!SONG.player1.startsWith('gf') && SONG.player1 != "bf-amogus") 
						schoolIntro(doof);
					else
						startCountdown();
				default:
	...
	override public function update(elapsed:Float)
	{
		...
		var accuracy = Math.round(((goodnotes - misses) / (goodnotes + misses)) * 100);
		...
		if (FlxG.keys.justPressed.U && SONG.player1 == "tankman")
		{
		...
		if (health <= 0 && praticemode == false)
		{
			...
			if (SONG.player1 == "tankman" || SONG.player1 == "pico" || SONG.player1 == "gf" || SONG.player1 == "gf-christmas")
			{
				FlxG.switchState(new GitarooPause());
			}
			else
	...
	private function keyShit():Void
	{
		...
		{
			if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && !boyfriend.animation.curAnim.name.endsWith('alt'))
			{
				boyfriend.playAnim('idle');
				if (SONG.player1.startsWith('gf'))
					boyfriend.dance();
